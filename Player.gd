...
func _physics_process(delta: float):
	...
	var is_idle = is_on_floor() and input_dir.length()<=0.0 and state != WALLRUNNING
	...
	if (wr_cooldown < 1.0):
		wr_cooldown += delta
	if Input.is_action_pressed(prefix+"jump") and !is_on_floor() and is_on_wall() and wr_cooldown >= 1.0 and (Input.get_action_strength(prefix+"left") or Input.get_action_strength(prefix+"right")):
		state = WALLRUNNING
